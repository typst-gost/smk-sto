...
#let smk-style(
  ...
  // Список использованных источников оформляется как обычный нумерованный
  // список: номер каждой записи — с абзацного отступа (красная строка), а
  // перенесённые строки начинаются от левого края; интервал между записями —
  // как между пунктами перечисления (межстрочный). Typst рендерит записи
  // библиографии блоками, к которым first-line-indent напрямую не применяется,
  // поэтому каждую запись (блок с автоматическими отступами above/below)
  // перестраиваем в абзац с красной строкой. Заголовок списка — блок с явными
  // above/below от `show heading` — не трогаем, чтобы сохранить его интервалы.
  show bibliography: it => {
    show block: b => if b.at("above", default: auto) == auto {
      par(first-line-indent: (amount: indent, all: true), b.body)
    } else {
      b
    }
    it
